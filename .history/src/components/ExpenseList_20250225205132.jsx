...
const ExpenseList = ({ expenses = [], onDeleteExpense, darkMode }) => {
  ...
  return (
    <motion.div
      ...
      className={`mt-6 p-4 rounded-lg shadow-md transition-all duration-300 ease-in-out ${darkMode ? "bg-gray-800 text-white border-gray-700" : "bg-white text-gray-800 border-gray-200"}`}
    >
              ...
              <ul className="pl-6 space-y-2">
                {categoryExpenses.map((expense) => (
                  ...
                  >
                    ...
                    <span className={`font-semibold ${darkMode ? "text-gray-200" : "text-gray-700"}`}>${expense.amount.toFixed(2)}</span>
                    <button
                      onClick={() => onDeleteExpense(expense._id)}
                      className="text-red-500 dark:text-red-400 hover:text-red-700 dark:hover:text-red-300 transition-all"
                    >
                      ❌
                    </button>
                  </motion.li>
